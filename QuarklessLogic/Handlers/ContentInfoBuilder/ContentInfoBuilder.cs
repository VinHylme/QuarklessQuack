...
namespace QuarklessLogic.Handlers.ContentInfoBuilder
{
	public class ContentInfoBuilder : IContentInfoBuilder
	{
		...
		{
			var hashtagsToUse = await _utilProviders.HashtagGenerator.SuggestHashtags(profileTopic, mediaTopic, hashtagPickAmount, medias);
			if (!hashtagsToUse.Any())
			{
				throw new Exception("Failed to find hashtags for image");
			}

			var caption = await _utilProviders.TextGenerator.GenerateCaptionByMarkovChain(mediaTopic,SecureRandom.Next(1, 2));
