...
namespace Quarkless.Base.Actions.Logic.Action_Instances
{
	internal class CreatePostAction : BaseAction, IActionCommit
	{
		...
		private async Task<(List<Meta<Quarkless.Models.SearchResponse.Media>>, MetaDataType)> GetMediaFromSearchType(SearchType searchTypeSelected)
		{
			switch (searchTypeSelected)
			{
				case SearchType.Google:
				{
					var googleResults = await _heartbeatLogic.GetMetaData<Quarkless.Models.SearchResponse.Media>(new MetaDataFetchRequest
					{
					...
					return (googleResults != null ? googleResults.ToList() : new List<Meta<Quarkless.Models.SearchResponse.Media>>(),
						MetaDataType.FetchMediaForSpecificUserGoogle);
				}
				case SearchType.YandexQuery:
				{
					var yandexResults = await _heartbeatLogic.GetMetaData<Quarkless.Models.SearchResponse.Media>(new MetaDataFetchRequest
					{
					...
					return (yandexResults != null ? yandexResults.ToList() : new List<Meta<Quarkless.Models.SearchResponse.Media>>(),
				...
				case SearchType.Instagram:
				{
					...
					var resultsFromMetaData = (await _heartbeatLogic.GetMetaData<Quarkless.Models.SearchResponse.Media>(new MetaDataFetchRequest
					{
					...
					{
						resultsFromMetaData = (await _heartbeatLogic.GetMetaData<Quarkless.Models.SearchResponse.Media>(new MetaDataFetchRequest
						{
					...
					return (resultsFromMetaData ?? new List<Meta<Quarkless.Models.SearchResponse.Media>>(), selectedMetaDataType);
				}
		...
		private TempSelect FindImageFromList(in List<Meta<Quarkless.Models.SearchResponse.Media>> mediaItems, MetaDataType selectedAction)
		{
		...
		private TempSelect FindVideoFromList(in List<Meta<Quarkless.Models.SearchResponse.Media>> mediaItems, MetaDataType selectedAction)
		{
			var selectedMedia = new TempSelect();
		...
		private TempSelect FindCarouselFromList(in List<Meta<Quarkless.Models.SearchResponse.Media>> mediaItems, MetaDataType selectedAction)
		{
			var selectedMedia = new TempSelect();
			var filterMediaSize = new Size(850, 850);
		...
		public async Task<ResultCarrier<EventActionModel>> PushAsync(DateTimeOffset executionTime)
		{
			...
			try
			{
				...
				var filterByMediaType = filteredResults.Select(meta =>
					new Meta<Quarkless.Models.SearchResponse.Media>(new Quarkless.Models.SearchResponse.Media
						{
				...
				if ((!filterByMediaType.Any() || !filterByMediaType.All(s=>s.ObjectItem.Medias.Any())))
				{
					...
					if (actionType == PostMediaActionType.Carousel)
					{
						...
						new Meta<Quarkless.Models.SearchResponse.Media>(new
							Quarkless.Models.SearchResponse.Media
							{
