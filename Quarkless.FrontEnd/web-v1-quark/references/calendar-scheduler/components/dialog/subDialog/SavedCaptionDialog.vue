    ...
    <div class="v-cal-dialog" v-if="isActive">
      ...
      <div class="section-start">
         <header class="v-cal-dialog-card__header">
            <h5 class="v-cal-dialog__title">Saved Captions</h5>
          </header>
          <section class="body-container">
            <div class="res_container">
              <div v-for="(caption,index) in captionLibrary.items" :key="index">
                <TextCard @click-select="selectedCaption" :rows="3" :isArray="false" :data="caption" :allowDelete="false" :allowEdit="false" icon="book-open" :date="formatedDate(caption.dateAdded)" :message="caption.caption"/>
              </div>
            </div>
          </section>
      </div>
...
export default {
  ...
  methods:{
    ...
    loadData(){
    this.captionLibrary.items = state.getters.UserCaptionLibrary(route.app.$route.params.id);
