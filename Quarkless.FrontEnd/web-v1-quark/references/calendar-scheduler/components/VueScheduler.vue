    ...
    <header class="v-cal-header">
      <div class="v-cal-header__actions">
        ...
        <div class="actions-right">
          <button class="v-cal-button" v-for="view in availableViews" v-bind:key="view" @click="switchView(view)" :class="{ 'v-cal-button--is-active': activeView === view }" >{{ labels[view] | capitalizeFirstLetter }}</button>
        </div>
      </div>
    ...
    export default {
        ...
        methods: {
            openEventViewDialog(event){
				EventDialog.view(event)
                .$on('event-deleted',(event) => {
                    this.$store.dispatch('DeleteEvent', event).then(res=>{
                ...
                .$on('enqueue-event',(event) => {
                    this.$store.dispatch('EnqueueEventNow', event).then(res=>{
                          Vue.prototype.$toast.open(
                            {
                               message: 'Posting Now!',
                               type: 'is-success'
                ...
                .$on('update-event', (event) => {
                    this.$store.dispatch('UpdateEvent', event.object_to_update).then(res=>{
                       Vue.prototype.$toast.open(
                            {
                               message: 'Updated!',
                               type: 'is-success'
            ...
            openMiniEventViewDialog(event){
                EventDialog.hoverView(event);
            },
            openEventDialog(data) {
                if ( !this.disableDialog ) {
                    ...
                    if ( data instanceof Date ) {
						...
						config.date = data.time;
                        config.startTime = data.time !== null ? moment(data.time, 'HH') : null;
						config.endTime = data.time !== null ? moment(data.time, 'HH').add(30, 'minutes') : null;
					}
            ...
            bindEvents() {
                ...
                EventBus.$on('event-clicked', (event) => {
                    ...
                    this.openEventViewDialog(event);
                });
                // EventBus.$on('event-hover', (event)=>{
                //     this.$emit('event-hover', event._e);
                //     this.openMiniEventViewDialog(event);
                // })
            },
            goToToday() {
            ...
            activeViewProps() {
                let props = {
                    ...
                    events: this.newEvents.filter( event => {
                        return event.startTime.isSame(this.activeDate, this.activeView);
                    })
				};
