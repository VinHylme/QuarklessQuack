    ...
    <section class="v-cal-content">
        ...
        <div class="v-cal-days">
            <div class="v-cal-times">
                ...
                <div class="v-cal-hour" :class="{ 'is-now': time.isSame(now, 'hour') }" v-for="(time,index) in times" v-bind:key="index">{{ time | formatTime(use12) }}</div>
            </div>
            <div class="v-cal-days__wrapper">
                <div class="v-cal-day v-cal-day--day" :class="{ 'is-today': day.isToday }" v-if="day !== null">
                        ...
                        <div class="v-cal-day__hour-content">
                            <div class="v-cal-event-list" :class="{'tiny-events': day.events.filter(e => !e.startTime).length > 2}">
                                <event-item
                                        ...
                                        :event="event"
                                        :use12="use12"
                                        @mouseover.stop="eventBus.$emit('event-hover', event)"
                    ...
                    <div class="v-cal-day__hour-block"
                         ...
                         :class="[ time.isSame(now, 'hour') ? 'is-now' : '', hourClass ]" v-for="(time,index) in day.availableTimes" v-bind:key="index">
    ...
    export default {
        ...
        methods: {
            timeClicked(data) {
				...
				let time = moment(data.time,'HH:mm').format('HH:mm');
				data.time =  moment(date + ' ' + time);
                EventBus.$emit('time-clicked', data)
            },
            buildCalendar() {
				let now = moment();
				const today = moment( this.activeDate );
                const dayEvents = this.events.filter( e => moment(e.startTime).isSame(today, 'day') )
                    .sort( (a, b) => {
