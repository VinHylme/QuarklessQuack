  ...
  <section class="v-cal-content">
    <div class="v-cal-weekdays">
      ...
      <div class="v-cal-weekday__wrapper">
        <div class="v-cal-weekday-item" v-for="(day,index) in days" :key="index">{{ day.d.format('ddd DD/MM') }}</div>
      </div>
    </div>
    <div class="v-cal-days">
      <div class="v-cal-times">
        <div class="v-cal-hour all-day">{{ allDayLabel }}</div>
        <div class="v-cal-hour" :class="{ 'is-now': time.isSame(now, 'hour') }" v-for="(time,index) in times" :key="index">{{ time | formatTime(use12) }}</div>
      </div>
      <div class="v-cal-days__wrapper">
        <div class="v-cal-day v-cal-day--week" v-for="(day,index) in days" :key="index" :class="{ 'is-today': day.isToday, 'is-disabled': day.isDisabled }">
          ...
          <div class="v-cal-day__hour-block"
				v-for="(time,index) in day.availableTimes" :key="index"
               :class="[ time.hour() === now.hour() ? 'is-now' : '', hourClass ]" 
			    @click="timeClicked({ date: day.d.toDate(), time: time })"
			> 
...
<script>
    ...
    export default {
        ...
        methods: {
            timeClicked(data) {
				data.time = moment(data.time,'HH:mm');
            ...
            buildCalendar() {
                ...
                do {
                    ...
                    const dayEvents = this.events.filter( e => e.startTime.isSame(day, 'day') )
                        .sort( (a, b) => {
